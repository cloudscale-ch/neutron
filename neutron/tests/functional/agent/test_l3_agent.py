...
class L3AgentTestFramework(base.BaseSudoTestCase):
    ...
    def get_expected_keepalive_configuration(self, router):
        ...
        return """vrrp_sync_group VG_VR_1{
    group  {
        VR_1
    }
    notify_master "%(ha_script_path)s primary"
    notify_backup "%(ha_script_path)s backup"
    notify_fault "%(ha_script_path)s fault"
}
vrrp_instance VR_1 {
    state BACKUP
    ...
    virtual_routes {
        ...
        8.8.8.0/24 via 19.4.4.4
            ...
            'ha_script_path': os.path.join(self.agent.conf.ha_confs_path,
                                           router.router_id,
                                           'primary-backup.sh')
        }
